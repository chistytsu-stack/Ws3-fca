...
module.exports.onChat = async ({ api, event, message, usersData }) => {
    try {
        ...
        if (body.startsWith("baby") || body.startsWith("bby") || body.startsWith("bot") || body.startsWith("jan") || body.startsWith("বট") || body.startsWith("janu")) {
            const arr = body.replace(/^\S+\s*/, "");

            const rawReplies = [
                "-amake dakla mone hoi?🙆",
                "Ja bolbi ekshate bole fel🤷",
                "Ey to ami e dike-🙋",
                "Yamete Kudasai~🐶",
                "Ha bolo ki bolba ami shuntesi👂",
                "ki problrm bby?🙂",
                "hellu kacher manush jon😘"
            ];
